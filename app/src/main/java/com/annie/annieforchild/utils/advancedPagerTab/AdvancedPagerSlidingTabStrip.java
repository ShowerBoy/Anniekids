package com.annie.annieforchild.utils.advancedPagerTab;
 ...
 * <p>
 * 添加apTabScrollLineWidth属性，改变底部滑动线长度，值越大width越小
 * 由两端两个实心圆和一条直线组成
 * 调整x轴位置改变长度
 * Update by wanglei on 2019/7/25.
 */
...
//    private int tabBackgroundResId;
    ...
    public AdvancedPagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        scrollLineWidth = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_IN, scrollLineWidth, dm);
        ...
        scrollLineWidth = a.getInteger(R.styleable.AdvancedPagerSlidingTabStrip_apTabScrollLineWidth, scrollLineWidth);
...
//        txtParams.addRule(RelativeLayout.CENTER_IN_PARENT);
    ...
    private void drawTabTextMode(Canvas canvas) {
        ...
        canvas.drawCircle(lineLeft + currentTextViewLeft + r + scrollLineWidth, height - indicatorHeight + r, r, rectPaint);
        canvas.drawCircle(lineLeft + currentTextViewRight - r - scrollLineWidth, height - indicatorHeight + r, r, rectPaint);

        canvas.drawRect(lineLeft + currentTextViewLeft + r + scrollLineWidth, height - indicatorHeight, lineLeft + currentTextViewRight - r - scrollLineWidth, height, rectPaint);
